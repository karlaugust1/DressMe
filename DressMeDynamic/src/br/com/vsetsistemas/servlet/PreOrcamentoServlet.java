 ...
 */
@WebServlet("/PreOrcamentoServlet")
public class PreOrcamentoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("utilizarPontos") != null) {
			if (request.getParameter("utilizarPontos").equals("true")) {
				...
				if (PreOrcamentoServlet.pedidoVenda.getCliente() != null
						&& PreOrcamentoServlet.pedidoVenda.getValorSubtotal() != 0) {

					ClienteSession session = new ClienteSession();
					Cliente c = session.obtainById(PreOrcamentoServlet.pedidoVenda.getCliente().getId());
					float porcentagemDesconto = (c.getQtdPontos() / 100);
					if (porcentagemDesconto > 0) {
						vlrDesconto = (PreOrcamentoServlet.pedidoVenda.getValorSubtotal() * porcentagemDesconto);
						vlrDesconto = vlrDesconto / 100;
						PreOrcamentoServlet.pedidoVenda.setDesconto(pedidoVenda.getDesconto() + vlrDesconto);
		...
		String nextJSP = "/inserirOrcamento.jsp";
